...
app.get('/tunnel_jwt_generator', (req, res) => {
  ...
  if (
    req.query.username === JWT_GENERATOR_USERNAME &&
    req.query.password === JWT_GENERATOR_PASSWORD
  ) {
    const jwtToken = jwt.sign({ token: VERIFY_TOKEN }, SECRET_KEY);
    res.status(200);
    res.send(jwtToken);
    return;
  }
